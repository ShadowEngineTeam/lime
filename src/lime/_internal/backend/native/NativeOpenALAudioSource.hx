...
class NativeOpenALAudioSource
{
	...
	{
		lime.utils.Log.error("NativeOpenALAudioSource does not support initializing from bytes. Use AudioSource.fromBytes() with the miniaudio backend.");
	}

	public function initFromFile(path:String):Void
	{
		lime.utils.Log.error("NativeOpenALAudioSource does not support initializing from a file path. Use AudioSource.fromFile() with the miniaudio backend.");
	}

	public function initFromAudioBuffer(sampleRate:Int, channels:Int, format:lime.media.MiniaudioFormat, pcmFrames:BytesData, pcmFrameCount:Int):Void
	{
		lime.utils.Log.error("NativeOpenALAudioSource does not support initializing from an AudioBuffer. Use AudioSource.fromAudioBuffer() with the miniaudio backend.");
	}

	public function readbackPcm():MiniaudioAudioSourceReadbackData
	{
		lime.utils.Log.error("NativeOpenALAudioSource does not support reading back pcm frames. Use the miniaudio backend.");
		return null;
	}
}
