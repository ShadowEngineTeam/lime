...
namespace lime
{
	...
	template <typename T> void DoDelete(T &item) {}
	...
	template <typename T_, int QBUF_SIZE_ = 16> class QuickVec
	{
		enum
		{
			QBufSize = QBUF_SIZE_
	  ...
	  public:
		...
		template <typename X_, typename D_> void Change(X_ inValue, D_ inDiff)
		{
			if (mSize == 0)
			{
		...
		inline T_ qpop() { return mPtr[--mSize]; }
