...
namespace lime
{
	...
	ImageDataView::ImageDataView(Image *image, Rectangle *rect)
	{
		...
		if (this->rect.y < 0)
			this->rect.y = 0;
		if (this->rect.x + this->rect.width > image->width)
			this->rect.width = image->width - this->rect.x;
		if (this->rect.y + this->rect.height > image->height)
			this->rect.height = image->height - this->rect.y;
