...
namespace lime
{
	...
	bool WEBP::Decode(Resource *resource, ImageBuffer *imageBuffer)
	{
		...
		if (!IMG_isWEBP((SDL_IOStream *)file.handle))
		{
			file.Close();
		...
		SDL_Surface *surface = IMG_LoadWEBP_IO((SDL_IOStream *)file.handle);
	...
	bool WEBP::Encode(ImageBuffer *imageBuffer, Bytes *bytes, int quality)
	{
		...
		if (bytes)
		{
			success = IMG_SaveWEBP_IO(surface, dst, false, quality);
