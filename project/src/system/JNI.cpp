...
namespace lime
{
	...
	template <typename OBJ> bool AbstractToObject(value inValue, OBJ *&outObj)
	{
		outObj = 0;
		...
		jclass getClass(JNIEnv *inEnv)
		{
			...
			switch (element)
			{
				case jniObjectString:
					...
					break;
				case jniObjectHaxe:
					name += "org/haxe/lime/HaxeObject";
					break;
				case jniValueObject:
					name += "org/haxe/lime/Value";
					break;
				case jniUnknown:
				case jniObject:
...
#define ARRAY_SET(PRIM, JTYPE, CREATE)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
	...
	const char *JNIParseType(const char *inStr, JNIType &outType, int inDepth = 0)
	{
		...
		{
			case 'B':
				outType = JNIType(jniByte, inDepth);
				return inStr;
			case 'C':
				outType = JNIType(jniChar, inDepth);
				return inStr;
			case 'D':
				outType = JNIType(jniDouble, inDepth);
				return inStr;
			case 'F':
				outType = JNIType(jniFloat, inDepth);
				return inStr;
			case 'I':
				outType = JNIType(jniInt, inDepth);
				return inStr;
			case 'J':
				outType = JNIType(jniLong, inDepth);
				return inStr;
			case 'S':
				outType = JNIType(jniShort, inDepth);
				return inStr;
			case 'V':
				outType = JNIType(jniVoid, inDepth);
				return inStr;
			case 'Z':
				outType = JNIType(jniBoolean, inDepth);
				return inStr;
			case '[':
	...
	bool HaxeToJNI(JNIEnv *inEnv, value inValue, JNIType inType, jvalue &out)
	{
		...
		else
		{
			switch (inType.element)
			{
				...
				case jniBoolean:
					out.z = (bool)val_number(inValue);
					return true;
				case jniByte:
					out.b = (int)val_number(inValue);
					return true;
				case jniChar:
					out.c = (int)val_number(inValue);
					return true;
				case jniShort:
					out.s = (int)val_number(inValue);
					return true;
				case jniInt:
					out.i = (int)val_number(inValue);
					return true;
				case jniLong:
					out.j = (long)val_number(inValue);
					return true;
				case jniFloat:
					out.f = (float)val_number(inValue);
					return true;
				case jniDouble:
					out.d = val_number(inValue);
					return true;
				case jniVoid:
					out.l = 0;
					return true;
