...
#	pragma pack(pop)
	...
	// __try must live in a function without C++ object unwinding (C2712),
	// which hxcpp's /EH compiler flags would otherwise require
	static void setLegacyThreadName(const ThreadNameInfo* _tn)
	{
		__try
		{
			RaiseException(0x406d1388
				, 0
				, sizeof(*_tn)/4
				, reinterpret_cast<const ULONG_PTR*>(_tn)
				);
		}
