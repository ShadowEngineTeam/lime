...
#define $A VK_COMPONENT_SWIZZLE_A
		...
		{ VK_FORMAT_BC6H_SFLOAT_BLOCK,         VK_FORMAT_BC6H_SFLOAT_BLOCK,        VK_FORMAT_UNDEFINED,           VK_FORMAT_UNDEFINED,                { $_, $_, $_, $_ } }, // BC6HS
...
#	error "Figure out KHR surface..."
	...
	VkResult SwapChainVK::createSwapChain()
	{
		...
		VkFormat surfaceFormat = srgb
			? s_textureFormat[m_colorFormat].m_fmtSrgb
			: s_textureFormat[m_colorFormat].m_fmt
			;

		// lime patch: the surface may not support the requested format (Android
		// surfaces commonly expose only RGBA8 while the bgfx default is BGRA8);
		// creating the swapchain with an unsupported format makes drivers
		// silently swap red/blue. Negotiate against the supported format list
		// and keep m_colorFormat in sync so render passes and readback match.
		{
			...
			&&  0 < numSurfaceFormats)
			{
				...
				if (VK_SUCCESS == result)
				{
					...
					{
						found = surfaceFormats[ii].format     == surfaceFormat
							&&  surfaceFormats[ii].colorSpace == surfaceColorSpace
							;
					}

					if (!found)
					{
						const TextureFormat::Enum fallbackFormats[] =
						{
							TextureFormat::RGBA8,
							TextureFormat::BGRA8,
						};

						for (uint32_t ff = 0; ff < BX_COUNTOF(fallbackFormats) && !found; ++ff)
						{
							const VkFormat fallbackFormat = srgb
								? s_textureFormat[fallbackFormats[ff] ].m_fmtSrgb
								: s_textureFormat[fallbackFormats[ff] ].m_fmt
								;

							for (uint32_t ii = 0; ii < numSurfaceFormats; ++ii)
							{
								if (surfaceFormats[ii].format     == fallbackFormat
								&&  surfaceFormats[ii].colorSpace == surfaceColorSpace)
								{
									BX_TRACE("Surface does not support requested back buffer format %d, falling back to %d."
										, m_colorFormat
										, fallbackFormats[ff]
										);
									m_colorFormat = fallbackFormats[ff];
									surfaceFormat = fallbackFormat;
									found = true;
									break;
								}
							}
