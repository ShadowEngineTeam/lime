...
#undef GET_CONFIG_ATTRIB
			...
			for (uint32_t retry = 0; retry < 4; ++retry)
			{
				...
				// First try: request ES 3.0, then fall back to ES 2.0
				uint32_t tryGlesVersion = (retry < 2) ? 30 : 20;

				attrs[numAttrs++] = EGL_RENDERABLE_TYPE;
				attrs[numAttrs++] = !!BGFX_CONFIG_RENDERER_OPENGL
					? EGL_OPENGL_BIT
					: (tryGlesVersion >= 30) ? EGL_OPENGL_ES3_BIT_KHR : EGL_OPENGL_ES2_BIT
					;
				...
				||  0 == numConfigs)
				{
					if (retry < 2)
					{
						// Retry ES 3.0 without MSAA first
						msaaSamples = 0;
					}
					else
					{
						// ES 2.0 attempt failed too; fall through to fatal
					}
					continue;
				}
...
#	endif // BX_PLATFORM_LINUX
			...
			const uint32_t contextVersionAttempts[] = { 30, 20 };
			uint32_t chosenGlesVersion = 0;

			for (uint32_t vv = 0; vv < BX_COUNTOF(contextVersionAttempts); ++vv)
			{
				chosenGlesVersion = contextVersionAttempts[vv];
...
#	else
					if (hasEglKhrCreateContext)
					{
						...
						bx::write(&writer, EGLint(chosenGlesVersion / 10), bx::ErrorAssert{});
...
#	endif // BX_PLATFORM_RPI
					...
					BX_TRACE("Failed to create EGL context version %d.%d with flags 0x%08x. Retrying without it!", chosenGlesVersion / 10, chosenGlesVersion % 10, flags);
				}
			}

			BGFX_FATAL(false, Fatal::UnableToInitialize, "Failed to create context.");
